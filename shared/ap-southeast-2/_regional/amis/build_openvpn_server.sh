...
# https://github.com/gruntwork-io/terraform-aws-service-catalog/blob/v0.95.0/modules/mgmt/openvpn-server/openvpn-server-ubuntu.pkr.hcl
#
...
eopdev_account_id="$(jq -r '."eopdev".id' "$git_repo_root/accounts.json")"
eopprod_account_id="$(jq -r '."eopprod".id' "$git_repo_root/accounts.json")"
eopstage_account_id="$(jq -r '."eopstage".id' "$git_repo_root/accounts.json")"
ami_account_ids="[\"$eopdev_account_id\",\"$eopprod_account_id\",\"$eopstage_account_id\"]"
...
function run {
  ...
  args=( \
    ...
    --var encrypt_kms_key_id="arn:aws:kms:ap-southeast-2:$shared_account_id:alias/ami-encryption" \
