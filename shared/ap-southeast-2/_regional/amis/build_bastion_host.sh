...
readonly PACKER_TEMPLATE_REPO_REF="v0.105.1"
readonly SERVICE_CATALOG_REF="v0.105.1"
...
dev_account_id="$(jq -r '."eopdev".id' "$git_repo_root/accounts.json")"
prod_account_id="$(jq -r '."eopprod".id' "$git_repo_root/accounts.json")"
stage_account_id="$(jq -r '."eopstage".id' "$git_repo_root/accounts.json")"
