...
(cd shared/ap-southeast-2/_regional/ecr-repos && terragrunt apply)
(cd shared/ap-southeast-2/_regional/shared-secret-resource-policies && terragrunt apply)
```
...
```
mkdir -p <REPLACE_WITH_NAME_OF_ACCOUNT>/ap-southeast-2/mgmt
cp -r dev/ap-southeast-2/mgmt/{vpc-mgmt,ecs-deploy-runner} <REPLACE_WITH_NAME_OF_ACCOUNT>/ap-southeast-2/mgmt
```
...
```
(cd <REPLACE_WITH_NAME_OF_ACCOUNT>/ap-southeast-2/mgmt && terragrunt run-all apply)
```
